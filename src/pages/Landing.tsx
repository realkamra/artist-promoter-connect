  ...
  return (
    <main className="min-h-screen overflow-hidden bg-background text-foreground">
      ...
      <section className="relative mx-auto grid max-w-6xl items-center gap-16 px-6 pb-24 pt-16 lg:grid-cols-[1.05fr_.95fr] lg:px-8 lg:pb-36 lg:pt-24">
        <div className="pointer-events-none absolute -left-24 top-0 size-96 rounded-full bg-primary/10 blur-3xl" aria-hidden="true" />
        ...
        <motion.div initial={{ opacity: 0, scale: .96 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: .7, delay: .15 }} className="relative">
          <div className="absolute -right-8 -top-8 size-24 rounded-full border border-blue/20 bg-blue/5" aria-hidden="true" />
          <div className="relative rounded-3xl border border-border bg-card p-5 shadow-2xl shadow-black/20 sm:p-7">
            ...
            <ol className="space-y-3 pt-5">
              {[{ name: "Maya Chen", type: "Electronic · Berlin", color: "bg-pink/15 text-pink", initials: "MC", score: "96%" }, { name: "Late Night Radio", type: "Indie · Global", color: "bg-blue/15 text-blue", initials: "LN", score: "91%" }, { name: "Juno Collective", type: "Alt pop · Toronto", color: "bg-yellow/15 text-yellow", initials: "JC", score: "88%" }].map((person) => <li key={person.name} className="flex items-center gap-3 rounded-2xl border border-border/70 bg-background/50 p-3.5"><div className={`flex size-11 items-center justify-center rounded-xl text-xs font-bold ${person.color}`}>{person.initials}</div><div className="min-w-0 flex-1"><p className="truncate text-sm font-bold">{person.name}</p><p className="mt-0.5 text-xs text-muted-foreground">{person.type}</p></div><div className="text-right"><p className="text-sm font-bold text-green">{person.score}</p><p className="text-[10px] text-muted-foreground">fit score</p></div></li>)}
            </ol>
